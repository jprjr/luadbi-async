...
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
  int n = lua_gettop (L);
  ...
  if (!mysql_real_connect
      (conn->mysql, host, user, password, db, port, unix_socket, client_flag))
    {
      lua_pushnil (L);
 ...
 */
static int
connection_connect_start (lua_State * L)
{
  int n = lua_gettop (L);
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
  ...
  mysql_options (conn->mysql, MYSQL_OPT_NONBLOCK, 0);
  conn->event =
    mysql_real_connect_start (&(conn->ret), conn->mysql, host, user, password,
 ...
 */
static int
connection_connect_cont (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
 ...
 */
static int
connection_get_event (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
  lua_pushnumber (L, convert_mysql_to_ev (conn->event));
 ...
 */
static int
connection_get_socket (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
 ...
 */
static int
connection_get_timeout (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
 ...
 */
static int
connection_autocommit (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
 ...
 */
static int
connection_rollback (lua_State * L)
{
  connection_t *conn =
    (connection_t *) luaL_checkudata (L, 1, DBD_MYSQL_CONNECTION);
  int err = 0;
...
int
dbd_mysql_connection (lua_State * L)
{
